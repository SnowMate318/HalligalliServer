...
자신의 과일 카드를 잘 내고, 조건에 맞을 때 종(벨)을 가장 빠르게 눌러서 카드를 얻는 것. 최종적으로 카드를 가장 많이 모은 사람이 승리합니다.

1. 바나나, 딸기, 라임, 자두 그림이 1~5개씩 그려져 있는 총 56장의 카드가 있습니다.

2. 카드를 잘 섞어 모든 플레이어에게 똑같이 나눠줍니다.

3. 각자 자신의 카드 더미를 보이지 않게 엎어둡니다.

4. 테이블 가운데에는 종을 둡니다.

5. 차례대로 자신의 카드 더미에서 한 장씩 앞면으로 뒤집어 중앙에 놓습니다.

5. 중앙에 공개된 카드들을 합쳐서 같은 과일이 정확히 5개가 되면(예: 바나나 3개 + 바나나 2개 = 5개)
누구든지 가장 먼저 종을 눌러야 합니다!

6. 종을 가장 먼저 누른 사람이 중앙에 나온 모든 카드를 가져가서 자신의 더미 맨 아래에 넣습니다.

7. 만약 잘못 눌렀다면, 자신의 카드에서 다른 플레이어들에게 한 장씩 벌칙으로 줍니다.
...
1. 로비
    1. 사용자는 다른 참여자가 접속할 수 있는 방을 생성할 수 있습니다.
    2. 사용자는 생성된 방의 정보를 조회할 수 있습니다.
    3. 생성된 방이 있을 경우, 사용자는 접속할 수 있습니다.
2. 방
    1. 최대 4명까지 입장 가능합니다.
    2. 참여자는 준비 상태를 on / off 할 수 있습니다.
    3. 참여자가 2명 이상이고, 모든 참여자가 준비된 상태일 경우 게임을 시작할 수 있습니다.
3. 게임
    1. 게임 시작 시 52장의 카드를 참여자들에게 등분 하여 나누어줍니다.
    2. 게임 내 모든 참여자들이 차례대로 카드를 테이블에 내는 것으로 턴이 진행됩니다.
    3. 참여자는 언제든 종을 누를 수 있으며, 조건 달성 여부에 따라 승리 및 패널티 로직이 실행됩니다.
...
* 객체 간 상호 호출 발생
   * 이를 해결하기 위해 중재자 패턴 활용
   * 요청 별로 관련있는 객체들을 이어주는 역할을 담당하는 인터페이스(Game) 추가  

---

### 요청 명세
#### 요청/응답 공통 규칙
- 모든 요청에는 `action: string`이 포함
- OK 응답 형식은 `{"response":"ok","action":"<요청 action>", ...}` 
- 에러 응답은 공통 포맷만 사용
